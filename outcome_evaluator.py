"""outcome_evaluator.py â€” Resolve JHL watch observations into market outcomes.
...
from eight_gates_zone_touch import resolve_zone_touch_lane
...
def _shadow_horizon(
    ...
    """Compute forward metrics for a shadow candidate observation.

    Parameters
    ----------
    candles          : OHLC rows after the reference bar
    reference_price  : close of the reference bar
    side             : 'LONG', 'SHORT', or 'NONE'
    invalidation_level : price level that would negate the claim (or None)

    Returns
    -------
    Dict with net_directional_return, realized_range, directional_efficiency,
    mfe, mae, hold_fail (if invalidation_level provided).

    Labels are always analytical; they are never trade win/loss or KNN inputs.
    """
    ...
    net_return = raw_return * direction if direction != 0 else raw_return

    path = sum(abs(r - l) for l, r in zip(closes, closes[1:]))
    efficiency = abs(final_close - reference_price) / path if path > 0 else 0.0

    if side == "LONG":
        mfe = max((h - reference_price) / divisor for h in highs)
        mae = min(0.0, min((lo - reference_price) / divisor for lo in lows))
    elif side == "SHORT":
        mfe = max((reference_price - lo) / divisor for lo in lows)
        mae = min(0.0, min((reference_price - h) / divisor for h in highs))
    else:
        mfe = max(abs(h - reference_price) / divisor for h in highs)
        mae = 0.0

    hold_fail: Optional[str] = None
    if invalidation_level is not None and side in {"LONG", "SHORT"}:
        if side == "LONG":
            violated = any(float(row[3]) < invalidation_level for row in candles)
        else:
            violated = any(float(row[2]) > invalidation_level for row in candles)
...
def _resolve_shadow(record: Dict[str, Any], now_epoch: int) -> Optional[Dict[str, Any]]:
    """Resolve observer/candidate outcomes for shadow Volatile and Trend Recovery records.

    Emits multi-horizon forward metrics at 15m, 30m, 60m, and 16-bar.
    Labels are analytical only â€” never trade win/loss or KNN inputs.
    """
    current = record.get("outcome") or {}
    if (
        str(current.get("status") or "").lower() == "resolved"
    ...
    observed = _parse_ts(record.get("ts"))
    ref_price = _number(record.get("reference_price"))
    ref_bar_ts = record.get("reference_bar_ts")

    if observed is None or ref_price is None:
        return {
            "status": "resolved",
            "label": None,
            "reason": "shadow_missing_reference_price_or_ts",
            "evaluated_at": datetime.now(timezone.utc).isoformat(),
        }

    if ref_bar_ts is not None:
        start_epoch = int(ref_bar_ts)
    else:
    ...
    horizons: Dict[str, Any] = {}
    for horizon_label, count in (("15m", 1), ("30m", 2), ("60m", 4), ("16bar", HORIZON_BARS)):
        if len(candles) >= count:
            horizons[horizon_label] = _shadow_horizon(
                candles[:count], ref_price, side, invalidation_level,
            )
...
def _resolve_native_lane(record: Dict[str, Any], now_epoch: int) -> Optional[Dict[str, Any]]:
    """Resolve native specialized-gate lane_observed records with zone-touch policy."""
    ...
    if observed is None:
        return {
            "status": "invalid",
            "label": None,
            "reason": "native_lane_missing_observed_at",
            "evaluated_at": datetime.now(timezone.utc).isoformat(),
        }

    start_epoch = int(
        observed.timestamp() // (INTERVAL_MINUTES * 60) * (INTERVAL_MINUTES * 60)
    )
    ...
    return resolve_zone_touch_lane(record, candles, HORIZON_BARS)
...
def _resolve(record: Dict[str, Any], now_epoch: int) -> Optional[Dict[str, Any]]:
    ...
    if record.get("record_type") == "lane_observed":
        return _resolve_native_lane(record, now_epoch)
...
def main() -> None:
    print(f"JHL outcome evaluator â€” {INTERVAL_MINUTES}m bars, {HORIZON_BARS}-bar horizon, refresh {REFRESH_SECONDS}s")
    while True:
        try:
            run_once()
        except Exception as exc:
