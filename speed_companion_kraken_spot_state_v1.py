...
def available_state(
    ...
    raw_phase = analyze_completed_candles(candles)

    raw_direction = str(raw_phase.get("direction") or "NONE").upper()
    raw_phase_name = str(raw_phase.get("phase") or "NONE").upper()

    try:
        raw_impulse_age = int(raw_phase.get("impulse_age_bars"))
    except (TypeError, ValueError):
        raw_impulse_age = None
    ...
    if (
        raw_phase_name in current_phases
        and raw_direction in {"LONG", "SHORT"}
        and raw_impulse_age is not None
        and raw_impulse_age <= ACTIVE_HORIZON_BARS
    ):
        effective_phase = raw_phase
        lifecycle_scope = "CURRENT_IMPULSE"
    else:
        effective_phase = {
            "direction": "NONE",
            "phase": "NONE",
        ...
        lifecycle_scope = "NO_CURRENT_IMPULSE"
    ...
    return {
        ...
        "speed_phase": effective_phase,
        "raw_speed_phase": raw_phase,
        "manual_review_only": True,
        "entry_authority": False,
    }
