"""
scanner.py — JHL Eight Gates Scanner
=====================================
...
Gate order:
  1. Data Integrity       — valid completed candles and trade data
  2. HTF Direction        — H1 EMA 50/200 order and slope
  3. Local Alignment      — M15 structure agrees with HTF
  4. Pressure Confirm     — delta_norm + CVD slope confirms direction
  5. Participation        — relative volume + ATR expansion, not thin
  6. Noise / No-Chase     — reject chop and extended late entries
  7. Trigger + Risk       — explicit trigger, ATR-buffered geometry
  8. Publish + Learn      — write brain observation, emit signal
"""
...
# ── Cycle ─────────────────────────────────────────────────────────────────────
SCAN_INTERVAL = 300          # 5 minutes — matches completed 15m bar boundary
TOP_N = 12                   # surface top 12 pairs
SCHEMA_VERSION = 4

# ── Gate thresholds (research-locked — do not change without outcome data) ────
MIN_SPEED = 0.50             # Delta 2.0 research: speed floor
MIN_ACCEL = 1.30             # 30% acceleration required
MIN_RISK_PCT = 0.0015        # 0.15% minimum stop distance — kills noise stops
ATR_STOP_MULTIPLIER = 1.0    # 1× ATR buffer on stops
...
# < 55 = BUILDING — suppressed from top-12

# ── ADA quarantine ────────────────────────────────────────────────────────────
ADA_QUARANTINE = {"ADA"}

# ── Pushover (optional — gracefully skipped if not configured) ────────────────
PUSHOVER_TOKEN = ""   # set in environment: PUSHOVER_TOKEN
PUSHOVER_USER = ""    # set in environment: PUSHOVER_USER
import os
...
def gate2_htf_direction(pair: PairContext) -> Tuple[Dict[str, Any], str]:
    """Gate 2: H1 EMA 50/200 — establish UP, DOWN, or NONE."""
    candles = pair.candles_15m
...
def gate4_pressure(pair: PairContext, htf_direction: str, delta_norm: float, cvd_slope: float) -> Dict[str, Any]:
    """Gate 4: Delta and CVD slope confirm direction (or BALANCED — warn not veto)."""
    if htf_direction == "UP":
    ...
    if not confirmed:
        # Balanced — don't veto, but mark as PASS with warning (reduces score)
...
def gate5_participation(pair: PairContext, atr: Optional[float]) -> Tuple[Dict[str, Any], float]:
    """Gate 5: Relative volume and ATR expansion — not thin or dead."""
    candles = pair.candles_15m
    if not candles:
    ...
    atr_pct = atr / price
    if atr_pct < 0.003:  # Less than 0.3% ATR — too compressed
...
def gate7_trigger_risk(
    ...
    # Minimum stop floor — reject noise stops
...
def _compute_score(gates: Dict[str, Any], delta_norm: float, rel_vol: float, reclaim: bool) -> int:
    ...
    if pressure.get("status") == "PASS":
        if pressure.get("reason") == "pressure_confirmed":
            ...
            score += 12  # balanced — partial credit
    ...
    if os.environ.get("GITHUB_FEED_PUSH", "1") != "0":
        try:
            ...
            if _token:
                ...
                print("  GITHUB_TOKEN not set — skipping feed push (set env var to enable)")
...
def main() -> None:
    print("JHL Eight Gates Scanner — 49 pairs, 5-min cycle, top-12 dynamic surfacing")
