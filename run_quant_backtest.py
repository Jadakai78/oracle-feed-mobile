...
def evaluate_rts_gate(card: dict) -> bool:
    ...
    # Filter out high liquidation risk zones
...
def evaluate_sequential_pipeline(card: dict) -> tuple[str, float]:
    ...
    # Checkpoint 2.5: RTS Liquidation & Regime Filter Integration
    if not evaluate_rts_gate(card):
        return "STAND_DOWN_RTS_RISK", 0.0
        
    # Checkpoint 3A: Anti-Delta Absorption Offense/Defense Check
    absorption_posture, absorption_r = evaluate_anti_delta_absorption(card)
    if "EXECUTE" in absorption_posture:
        delta_tag = review.get("delta_net_state")
        review["speed_phase"] = f"ABSORPTION_{delta_tag}"
        return absorption_posture, absorption_r
        
    # Checkpoint 3B: Core Momentum Clusters (Cluster A & B)
    speed = review.get("speed_phase")
    cvd = review.get("cvd_slope_state")
    reclaim = review.get("reclaim_confirmed")
    ...
    if is_cluster_a or is_cluster_b:
...
def run_sequential_simulation():
    ...
    print("Running 25,000 snapshots with RTS integration & tuned absorption weights...")
    ...
    for _ in range(25000):
        ...
        rts_state = random.choices(rts_states, weights=[0.60, 0.30, 0.10])[0]
    ...
    print("\n" + "=" * 80)
    print("      TUNED PIPELINE + ANTI-DELTA + RTS QUANT EXPECTANCY REPORT      ")
    print("=" * 80)
    ...
    json_path = "tuned_pipeline_rts_quant_report.json"
    csv_path = "tuned_pipeline_rts_quant_report.csv"
