...
import eg_trend_recovery
from specialized_gates_ledger import append_lane_once
...
def _emit_eg_trend_shadow(pair: PairContext, result: Dict[str, Any]) -> None:
    """Evaluate EG Trend Recovery in shadow mode only; never alters live output."""
    try:
        if result.get("rejected_at"):
            return

        atr = _number(result.get("atr"), 0.0)
        if atr <= 0:
            return

        lane = eg_trend_recovery.build_lane(
            pair=result["pair"],
            candles=pair.candles_15m or [],
            htf_direction=str(result.get("htf_direction") or "NONE"),
            delta_norm=_number(result.get("delta_norm"), 0.0),
            rel_vol=_number(result.get("rel_vol"), 0.0),
            atr=atr,
            observed_at=result.get("ts"),
        )

        if lane and append_lane_once(lane):
            print(
                f"  EG_TREND_SHADOW {lane['pair']} {lane['side']} "
                f"score={lane.get('score', '?')} "
                f"id={lane.get('observation_id', '?')[:8]}"
            )
    except Exception as exc:
        print(f"  EG_TREND_SHADOW_ERROR {pair.symbol}: {type(exc).__name__}: {exc}")
...
def _maybe_send_pushover_test() -> None:
    """Send exactly one test notification when TEST_ALERTS=1 in .env."""
    if os.environ.get("TEST_ALERTS", "0").strip() != "1":
        return
    if not PUSHOVER_TOKEN or not PUSHOVER_USER:
        print("PUSHOVER_TEST_SKIPPED credentials_not_loaded")
        return
    _send_pushover(
        "JHL Pushover test",
        "Pushover is connected. Production alerts remain limited to EXECUTION_ELIGIBLE setups.",
    )
    print("PUSHOVER_TEST_SENT")
...
def run_cycle() -> None:
    ...
    for pair in active_pairs:
        try:
            ...
            _emit_eg_trend_shadow(pair, result)
            status_line = (
